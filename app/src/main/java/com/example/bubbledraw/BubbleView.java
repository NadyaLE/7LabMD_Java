...
//настройка проекта "разноцветные пузырьки"
...
public class BubbleView extends ImageView implements View.OnTouchListener {
    ...
    //описание того, какие действия нужны когда вызывается поток Runnable r
    ...
    //рисование на устройстве с помощью метода onDraw()
    protected void onDraw(Canvas canvas) {
        for (Bubble b: bubbleList)
            b.draw(canvas);
            h.postDelayed(r, delay);
    }
    //тестовая функция для того, чтобы понять рисуются ли пузырьки
    ...
    //значения пузырьков, их конструктор для изменения цвета и скорости
